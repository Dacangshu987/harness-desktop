...
/** 安装 profile 依赖：优先 pnpm（profile 是 pnpm 生态，插件市场用 pnpm 管树），
 *  失败回退 npm；都失败回退「应用捆绑目录复制闭包」。绝不用 junction。 */
function installProfileDeps(nodeBin, profileDir, bundledNodeModules, profileNodeModules, onLine, allowInstall) {
  if (!allowInstall) return false; // 服务已运行且 node_modules 已有内容时，不折腾
  if (!nodeBin) return false;
  if (pnpmInstallProfile(nodeBin, profileDir, onLine)) return true;
  if (npmInstallProfile(nodeBin, profileDir, onLine)) return true;
  // 拷贝闭包兜底：生成独立真实目录，保证内置插件可用（插件市场受限但可跑）。
  try {
